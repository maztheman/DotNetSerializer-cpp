...
std::shared_ptr<CDotNetField> CDotNetFileReader::ReadPrimative(EDataType eDataType)
{
	...
	case eDataType_String:
		{
			auto pValue = std::make_shared<CStringField>();
	...
	case eDataType_Int16:
		{
			auto pIntField = std::make_shared<CInt16Field>();
	...
	case eDataType_Int32:
		{
			auto pIntField = std::make_shared<CInt32Field>();
	...
	case eDataType_Int64:
		{
			auto pIntField = std::make_shared<CInt64Field>();
	...
	case eDataType_DateTime:
		{
			auto pDateField = std::make_shared<CDateTimeField>();
	...
	case eDataType_Double:
		{
			auto pDoubleField = std::make_shared<CDoubleField>();
	...
	case eDataType_Boolean:
		{
			auto p = std::make_shared<CBooleanField>();
...
std::shared_ptr<CDotNetClass> CDotNetFileReader::ReadClass(bool bSystemClass = false)
{
	uint32_t id = m_Stream.ReadUInt32();
	...
	int32_t nFieldCount = oClass.GetFieldCount();
	if (nFieldCount <= 0) 
	{
		return pNewClass;
	}
	...
	for(auto& field : oClass.Fields())
	{
		m_Stream.ReadString(field.Name());
	}
	for(auto& field : oClass.Fields())
	{
		field.SetSchemaDataType(static_cast<ESchemaDataType>(m_Stream.ReadByte()));
	}
	for(auto& field : oClass.Fields())
	{
...
std::shared_ptr<CMemberReferenceField> CDotNetFileReader::ReadMemberReference()
{
	auto pMemberRefField = std::make_shared<CMemberReferenceField>(m_Stream.ReadUInt32());
	m_arNumber9.push_back(pMemberRefField);
	return pMemberRefField;
}

std::shared_ptr<CDotNetClass> CDotNetFileReader::ReadArray()
{
	...
	while(bContinue) 
	{
		...
		case eSchemaType_ArrayFiller32b:
			{
				uint32_t nFiller = m_Stream.ReadUInt32();
				nIndex += nFiller;
				nField += nFiller;
			}
			break;
		case eSchemaDataType_Array:
			...
			break;
		default:
			m_Log << "ReadArray, Unhandled schema type " << EnumToString(eSchemaType) << " (" << (int)eSchemaType << ")" << std::endl;
			break;
		};
...
std::shared_ptr<CDotNetField> CDotNetFileReader::ReadField()
{
	...
	case eSchemaType_BinaryObjectString:
			{
				auto pSField = std::make_shared<CStringField>();
	...
	case eSchemaType_ObjectNull://nop?
		pField = std::make_shared<CStringField>();
		break;
	case eSchemaType_ArrayFiller8b:
...
void CDotNetFileReader::ReadEnd()
{
	...
	std::string sName;
	sName.reserve(256);
	for(auto& pField : m_arNumber9)
	{
		auto pParent = pField->GetParent();
		auto itFound = m_mpClasses.find(pField->GetReferenceID());
		if (itFound != m_mpClasses.end())
		{
			...
			auto pNewClass = std::make_shared<CUserClassField>(itFound->second);
...
void CDotNetFileReader::ReadAssembly()
{
	...
	m_Log << "Assembly(" << pAssembly->GetID() << ") " << pAssembly->GetName() << std::endl;
}

std::shared_ptr<CDotNetClass> CDotNetFileReader::ReadArrayItem()
{
	...
	uint32_t schmeaObjectID = m_Stream.ReadUInt32();
...
std::shared_ptr<CDotNetClass> CDotNetFileReader::Deserialize()
{
